...
f.addElement("request_sdate", null, "hname:'수강신청시작(일)'");
f.addElement("request_shour", null, "hname:'수강신청시작(시)'");
f.addElement("request_smin", null, "hname:'수강신청시작(분)'");
f.addElement("request_edate", null, "hname:'수강신청종료(일)'");
f.addElement("request_ehour", null, "hname:'수강신청종료(시)'");
f.addElement("request_emin", null, "hname:'수강신청종료(분)'");
...
if(m.isPost() && f.validate()) {
	...
	*/
	...
	if(!alog.add(userId, newId, "C", "과정등록", f)) {
		Malgn.errorLog("{course.course_insert} actionLog add error : site_id = " + siteId + ", user_id = " + userId + ", course_id = " + newId);
	}
