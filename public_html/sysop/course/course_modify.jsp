...
boolean isRegular = "R".equals(info.s("course_type"));
String requestSdate = "";
String requestShour = "";
String requestSmin = "";
String requestEdate = "";
...
if(isRegular) {
	String requestSdatetime = info.s("request_sdate");
	String requestEdatetime = info.s("request_edate");
	if(!"".equals(requestSdatetime)) {
		requestSdate = requestSdatetime.substring(0,8);
		requestShour = requestSdatetime.substring(8,10);
		requestSmin = requestSdatetime.substring(10,12);
		requestEdate = requestEdatetime.substring(0,8);
		requestEhour = requestEdatetime.substring(8,10);
		requestEmin = requestEdatetime.substring(10,12);
	}
}
...
info.put("request_sdate", Malgn.time("yyyy-MM-dd", requestSdate));
info.put("request_shour", requestShour);
info.put("request_smin", requestSmin);
info.put("request_edate", Malgn.time("yyyy-MM-dd", requestEdate));
info.put("request_ehour", requestEhour);
info.put("request_emin", requestEmin);
...
f.addElement("request_sdate", Malgn.time("yyyy-MM-dd", requestSdate), "hname:'수강신청시작(일)'");
f.addElement("request_shour", requestShour, "hname:'수강신청시작(시)'");
f.addElement("request_smin", requestSmin, "hname:'수강신청시작(분)'");
f.addElement("request_edate", Malgn.time("yyyy-MM-dd", requestEdate), "hname:'수강신청종료(일)'");
f.addElement("request_ehour", requestEhour, "hname:'수강신청종료(시)'");
f.addElement("request_emin", requestEmin, "hname:'수강신청종료(분)'");
...
if(m.isPost() && f.validate()) {
	...
	*/
	...
	if("R".equals(f.get("course_type"))) { //정규
		...
		course.item("request_sdate", Malgn.time("yyyyMMdd", f.get("request_sdate")) + f.get("request_shour") + f.get("request_smin") + "00");
		course.item("request_edate", Malgn.time("yyyyMMdd", f.get("request_edate")) + f.get("request_ehour") + f.get("request_emin") + "00");
	...
	if(!alog.add(userId, id, "U", "과정정보수정", info, f)) {
	...
	if(isUpload) {
		...
		catch(Exception e) { m.errorLog("Exception : 파일 리사이징 처리 중 오류 발생 " , e); }
	}
