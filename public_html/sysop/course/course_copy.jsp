...
//폼체크
if(isRegular) {
	f.addElement("request_sdate", null, "hname:'수강신청시작(일)', required:'Y'");
	f.addElement("request_shour", null, "hname:'수강신청시작(시)'");
	f.addElement("request_smin", null, "hname:'수강신청시작(분)'");
	f.addElement("request_edate", null, "hname:'수강신청종료(일)', required:'Y'");
...
if(m.isPost() && f.validate()) {
	...
	//과정일자설정
	String requestSdate = Malgn.time("yyyyMMdd", f.get("request_sdate")) + f.get("request_shour") + f.get("request_smin") + "00";
	String requestEdate = Malgn.time("yyyyMMdd", f.get("request_edate")) + f.get("request_ehour") + f.get("request_emin") + "00";
	if(14 != requestSdate.length()) requestSdate = "";
	if(14 != requestEdate.length()) requestEdate = "";
	course.setCopyDate(
		requestSdate, requestEdate
		, Malgn.time("yyyyMMdd", f.get("study_sdate"))
		, Malgn.time("yyyyMMdd", f.get("study_edate"))
		, f.getInt("lesson_day")
	);
