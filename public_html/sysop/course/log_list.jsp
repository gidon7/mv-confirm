...
//접근권한
if(!"S".equals(userKind)) { m.jsError("접근 권한이 없습니다."); return; }
...
lm.addWhere("a.module = 'course'");
lm.addSearch("a.action_type", f.get("s_type"));
if(!"".equals(f.get("s_field"))) lm.addSearch(f.get("s_field"), f.get("s_keyword"), "LIKE");
else if("".equals(f.get("s_field")) && !"".equals(f.get("s_keyword"))) {
	lm.addSearch("a.action_desc,a.before_info,a.after_info,a.module_id", f.get("s_keyword"), "LIKE");
}
...
while(list.next()) {
	list.put("action_type_conv", Malgn.getItem(list.s("action_type"), actionLog.types));
	list.put("module_conv", Malgn.getItem(list.s("module"), actionLog.modules));
...
//출력
p.setLayout(ch);
p.setBody("course.log_list");
p.setVar("p_title", "과정로그관리");
p.setVar("query", m.qs());
...
p.setLoop("types", Malgn.arr2loop(actionLog.types));
p.setLoop("modules", Malgn.arr2loop(actionLog.modules));
p.setLoop("status_list", Malgn.arr2loop(actionLog.statusList));
