...
//기록-개인정보조회
if("".equals(m.rs("mode")) && info.size() > 0 && !isBlindUser) _log.add("V", Menu.menuNm, info.size(), inquiryPurpose, info);
...
while(files.next()) {
	...
	files.put("filename", m.htt(files.s("filename").replaceAll(allowRegexr, "&lt;$1$2&gt;")));
	files.put("filename", files.s("filename").replaceAll(" on([^\\t\\n\\f\\- \\/>\"'=]+\\s*)=", " on-$1="));
