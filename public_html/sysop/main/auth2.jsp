...
if(m.isPost() && f.validate()) {
    uinfo = user.find("user_nm = '" + userName + "' AND login_Id = '" + loginId + "' AND status = 1 AND site_id = " + siteId + " AND fail_cnt < 5");
    if(!uinfo.next()) { m.jsAlert("최고관리자에게 문의 부탁드립니다."); return; }
    ...
    if("E".equals(auth2Type) || "S".equals(auth2Type)) {
        ...
        if(!authNo.equals(sessAuthNo)) {
			...
			if(failCnt >= siteinfo.i("login_block_cnt")) {
				m.setSession("AUTH_NO_OTP", "");
				m.setSession(auth2Type + "_SENDDATE_OTP", "");

				m.jsAlert("인증에 " + siteinfo.i("login_block_cnt") + "회 이상 실패하여 로그인이 제한되었습니다.");
				m.jsReplace("logout.jsp", "parent");
				return;
			} else {
	            m.jsAlert("인증번호가 일치하지 않습니다." + (0 < siteinfo.i("login_block_cnt") ? "\\n인증번호 " + siteinfo.i("login_block_cnt") + "회 연속 오류 시 로그인이 제한됩니다." : ""));
	            return;
			}
        }
    ...
    if(!user.update("id = " + uinfo.i("id") + " AND site_id = " + uinfo.i("site_id"))) {
