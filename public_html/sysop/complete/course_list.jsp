...
if("excel".equals(m.rs("mode"))) {
	ExcelWriter ex = new ExcelWriter(response, "수료관리(" + m.time("yyyy-MM-dd") + ").xls");
	ex.setData(list, new String[] { "__ord=>No", "onoff_type_conv=>과정구분", "year=>년도", "step=>기수", "cate_name=>카테고리", "course_nm=>과정명", "user_cnt_conv=>수강생수", "complete_cnt=>수료자수", "complete_rate_conv=>수료율", "study_sdate_conv=>학습시작일", "study_edate_conv=>학습종료일", "close_conv=>상태"}, "과정개설관리(" + m.time("yyyy-MM-dd") +")");
	ex.write();
	return;
}
