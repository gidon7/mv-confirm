...
if("mod".equals(m.rs("mode")) && m.isPost()) {
	DataSet surveys = f.getArrList(
		"id,apply_type"
		+ ",open_date,open_date_hour,open_date_min"
	...
	while(surveys.next()) {
		...
		String openDate = !"".equals(surveys.s("open_date")) ? Malgn.time("yyyyMMdd", surveys.s("open_date")) + surveys.s("open_date_hour") + surveys.s("open_date_min") + "00" : "";

		DataSet courseModules = courseModule.find("course_id = " + courseId + " AND module = 'survey' AND module_id = " + surveys.i("id") + "");
		courseModules.next();

		if(!openDate.equals(courseModules.s("open_date"))) courseModule.item("open_date", openDate);
...
while(list.next()) {
	...
	list.put("open_date_conv", Malgn.time("yyyy-MM-dd", list.s("open_date")));
	list.put("open_date_hour", !"".equals(Malgn.time("HH", list.s("open_date"))) ? Malgn.time("HH", list.s("open_date")) : "00");
	list.put("open_date_min", !"".equals(Malgn.time("mm", list.s("open_date"))) ? Malgn.time("mm", list.s("open_date")) : "00");
