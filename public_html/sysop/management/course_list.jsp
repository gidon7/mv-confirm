...
lm.setFields("a.*"
	...
	+ ", (SELECT COUNT(*) FROM " + courseUser.table + " cu "
		...
		+ " WHERE cu.course_id = a.id AND cu.status != -1 "
	+ " ) user_cnt "
...
if(!"".equals(f.get("s_req_sdate"))) lm.addWhere("a.request_edate >= '" + m.time("yyyyMMddHHmmss", f.get("s_req_sdate")) + "'");
if(!"".equals(f.get("s_req_edate"))) lm.addWhere("a.request_sdate <= '" + m.time("yyyyMMddHHmmss", f.get("s_req_edate")) + "'");
...
if(f.getInt("s_progress") == 1)	lm.addWhere("a.course_type != 'A' AND a.request_sdate > '" + now + "'");
else if(f.getInt("s_progress") == 2) lm.addWhere("a.course_type != 'A' AND (a.request_sdate <= '" + today + "' AND a.request_edate >= '"+ now +"')");
...
while(list.next()) {
	...
	else if(14 == list.s("request_sdate").length() && 0 < m.diffDate("S", now, list.s("request_sdate"))) list.put("progress_conv", "대기");
	...
	else if(14 == list.s("request_sdate").length() && 14 == list.s("request_edate").length() && 0 >= m.diffDate("S", now, list.s("request_sdate")) && 0 <= m.diffDate("S", now, list.s("request_edate"))) list.put("progress_conv", "신청");
	...
	list.put("request_sdate_conv", m.time("yyyy.MM.dd HH:mm", list.s("request_sdate")));
	list.put("request_edate_conv", m.time("yyyy.MM.dd HH:mm", list.s("request_edate")));
