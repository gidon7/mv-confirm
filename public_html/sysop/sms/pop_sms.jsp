...
f.addElement("reservation_yn", null, "hname:'예약여부'");
f.addElement("send_date", m.time("yyyy-MM-dd"), "hname:'발송시기(날짜)'");
f.addElement("send_hour", m.time("HH"), "hname:'발송시기(시)'");
f.addElement("send_minute", m.time("mm"), "hname:'발송시기(분)'");

//변수
DataSet respData = new DataSet();
...
if(m.isPost() && f.validate()) {
	...
	//변수
	String sendDate = "Y".equals(f.get("reservation_yn")) ? m.time("yyyyMMdd", f.get("send_date")) + f.get("send_hour") + f.get("send_minute") + "00" : null;
	...
	while(users.next()) {
		...
		mobile = "";
		mobile = !"".equals(users.s("mobile")) ? users.s("mobile") : "";
		...
		if(sms.isMobile(mobile) && (!isAd || (isAd && users.b("sms_yn")))) {
			...
			if(smsUser.insert()) {
				if(isSend) {
					if("Y".equals(f.get("reservation_yn"))) {
						sms.reserveSend(mobile, f.get("sender"), (isAd ? "(광고) " : "") + f.get("content"), sendDate, newId);
					} else {
						sms.send(mobile, f.get("sender"), (isAd ? "(광고) " : "") + f.get("content"), "", newId);
					}
				}
				sendCnt++;
			}
...
while(users.next()) {
	users.put("s_value", !"".equals(users.s("mobile")) ? "(" + users.s("mobile") + ")" : "(-)" );
	//users.put("stype_yn", !"Y".equals(users.s("email_yn")) ? "[수신거부]" : "");
}

//기록-개인정보조회
_log.add("V", "SMS발송", users.size(), inquiryPurpose, users);
