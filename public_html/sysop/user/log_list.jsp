...
if("excel".equals(m.rs("mode"))) {
    ExcelWriter ex = new ExcelWriter(response, "회원이력관리(" + m.time("yyyy-MM-dd") + ").xls");
    ex.setData(list, new String[] { "__ord=>No", "user_nm=>회원", "login_id=>로그인아이디", "before_info=>기존정보", "after_info=>변경후정보", "manager_nm=>실행자", "manager_login_id=>실행자로그인아이디", "reg_date=>등록일" }, "회원이력관리(" + m.time("yyyy-MM-dd") + ")");
    ex.write();
    return;
}
