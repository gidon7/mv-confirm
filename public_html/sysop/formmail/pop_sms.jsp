...
//기록-개인정보조회
if(finfo.size() > 0 && !isBlindUser) _log.add("V", "게시판목록", finfo.size(), inquiryPurpose, finfo);
			...
			if(smsUser.insert()) {
				if(isSend) {
					if("Y".equals(f.get("reservation_yn"))) {
						sms.reserveSend(mobile, f.get("sender"), f.get("content"), sendDate, newId);
					} else {
						sms.send(mobile, f.get("sender"), f.get("content"), "", newId);
					}
				}
				sendCnt++;
			}
