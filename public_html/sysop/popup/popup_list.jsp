...
lm.setOrderBy(!"".equals(m.rs("ord")) ? m.rs("ord") : "a.sort ASC, a.reg_date DESC");
...
if(m.isPost() && "sort".equals(mode)) {
	String[] idx = f.getArr("id");
	for(int i = 0; idx != null && i < idx.length; i++) {
		popup.clear();
		popup.item("sort", i + 1);
		if(!popup.update("id = " + idx[i] + "")) {
			m.jsAlert("순서를 수정하는 중 오류가 발생했습니다.");
			return;
		}
	}
	m.jsReplace("popup_list.jsp?" + m.qs("mode,id"), "parent");
	return;
}
