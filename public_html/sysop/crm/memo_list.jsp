...
//기록-개인정보조회
if("".equals(m.rs("mode")) && info.size() > 0 && !isBlindUser) _log.add("V", "회원조회", info.size(), inquiryPurpose, info);
...
catch(RuntimeException re) { m.errorLog("RuntimeException : 파일 리사이징 처리 중 오류 발생 "); return; }
catch(Exception ex) { m.errorLog("Exception : 파일 리사이징 처리 중 오류 발생 "); return; }
