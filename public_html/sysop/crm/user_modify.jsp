...
SmsTemplateDao smsTemplate = new SmsTemplateDao();
...
if("sms_passwd".equals(m.rs("mode"))) {

	//메일
	if(sms.isMobile(info.s("mobile"))) {
...
//			smsTemplate.sendSms(siteinfo, info, "findpw_newpw", p);
		sms.send(info.s("mobile"), siteinfo.s("sms_sender"), "[" + siteinfo.s("site_nm") + "] " + "새로운 비밀번호는 " + newPasswd + " 입니다.");
		sms.insertSms(siteId, -9, siteinfo.s("sms_sender"),  "[" + siteinfo.s("site_nm") + "] " + "새로운 비밀번호는 " + newPasswd + " 입니다.", info);
...
//	f.addElement("passwd_date", m.time("yyyy-MM-dd", info.s("passwd_date")), "hname:'비밀번호 변경안내일'");
	...
	if(!"".equals(info.s("guardian_mobile"))) guardianMobile = info.s("guardian_mobile");
	...
	f.addElement("guardian_nm", info.s("guardian_nm"), "hname:'보호자 명'");
	f.addElement("guardian_mobile", guardianMobile, "hname:'보호자 휴대전화'");
	f.addElement("guardian_type", info.s("guardian_type"), "hname:'보호자 구분'");
	f.addElement("guardian_agree_yn", info.s("guardian_agree_yn"), "hname:'보호자 동의여부'");
}
...
if(m.isPost() && f.validate()) {
	...
	//비밀번호수정/세션삭제
		...
		//제한-비밀번호
		if(!f.get("passwd").matches("^(?!.*([A-Za-z])\\1)(?!.*(\\d)\\2)(?!.*([\\W_])\\3)(?=.*?[A-Za-z])(?=.*?[0-9])(?=.*?[\\W_]).{9,}$")) {
			m.jsAlert("비밀번호는 영문, 숫자, 특수문자 조합(연속된 문자, 반복된 문자나 숫자는 제외) 9자 이상 입력하세요.");
			return;
		}
	...
	user.item("guardian_nm", f.get("guardian_nm"));
	user.item("guardian_mobile", !"".equals(f.get("guardian_mobile")) ? f.get("guardian_mobile") : "");
	user.item("guardian_type", f.get("guardian_type"));
	...
	//기록-개인정보조회
	if(!isBlindUser) _log.add("U", "회원정보 수정", 1, inquiryPurpose, info);
...
info.put("status_conv", m.getItem(info.s("status"), user.statusList));
info.put("minor_user_block", "Y".equals(info.s("minor_yn")));
